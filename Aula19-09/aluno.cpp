...
void salvarArquivo(){
    ...
    arquivo.open("alunos.txt",ios::out|ios::app); //out é escrita e app é para não apagar os dados do arquivo escrito.
    map<string, notas>::iterator it;

    for(it = aluno.begin(); it != aluno.end(); it++){
        arquivo << "Aluno: " << it-> first << endl;
        arquivo << "Nota 01 " << it-> second.nota01 << endl;
        arquivo << "Nota 02 " << it-> second.nota02 << endl;
        arquivo << "Nota 03 " << it-> second.nota03 << endl;
    }
}
